...
public class ConsumerConfig {
    ...
    @Bean("reliableQueue")
    public Queue queue() {
        return new Queue("SEN_RELIABLE_SEND_QUEUE", true, false, false, new HashMap<>());
    }

    @Bean("directExchange")
    public DirectExchange exchange() {
        return new DirectExchange("SEN_RELIABLE_SEND_EXCHANGE", true, false, new HashMap<>());
    }

    @Bean
    public Binding binding(@Qualifier("reliableQueue") Queue queue, @Qualifier("directExchange") DirectExchange exchange) {
        return BindingBuilder.bind(queue).to(exchange).with("rabbit.tech");
    }
    ...
    public MessageListenerContainer messageListenerContainer(ConnectionFactory connectionFactory) {
        ...
        messageListenerContainer.setQueueNames("SEN_RELIABLE_SEND_QUEUE");
        // 消费者并发数
